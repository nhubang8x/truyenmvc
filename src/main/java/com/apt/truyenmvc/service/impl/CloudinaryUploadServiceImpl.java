...
import com.apt.truyenmvc.service.CloudinaryUploadService;
...
public class CloudinaryUploadServiceImpl implements CloudinaryUploadService {
    ...
    public String upload(MultipartFile sourceFile) {
        try {
            ...
            Map<String, Object> result = cloudinary.uploader().upload(sourceFile.getBytes(), params);
    ...
    public String upload(MultipartFile sourceFile, String fileName) {
        try {
            ...
            Map<String, Object> result = cloudinary.uploader().upload(sourceFile.getBytes(), params);
            //Lấy đường dẫn ảnh vừa upload
            String url = cloudinary.url().generate("truyenmvc/" + fileName);
            return url;
        } catch (IOException e) {
            return null;
        }
